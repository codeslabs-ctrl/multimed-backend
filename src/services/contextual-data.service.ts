...
export class ContextualDataService {
  ...
  private async obtenerHistorialConsultas(
    ...
    try {
      ...
      const result = await client.query(
        `SELECT h.*, c.tipo_consulta FROM historico_pacientes h
      ...
      const historial = result.rows.map((historial: any) => ({
        ...
        tratamiento_cumplido: historial.tratamiento_cumplido || '',
        evaluacion_subjetiva: historial.evaluacion_subjetiva || '',
        evaluacion_complementaria: historial.evaluacion_complementaria || '',
